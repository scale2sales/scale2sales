...
async function getDashboardData() {
  ...
  const [
    ...
    supabase.from('conversations').select('id, title, created_at, project_id').eq('organization_id', orgId).order('created_at', { ascending: false }).limit(5),
  ])
...
export default async function DashboardPage() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: `${monthlyMessages} / ${planLimit}`,
      icon: '📊',
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {monthlyMessages.toLocaleString()} of {planLimit.toLocaleString()} messages used
            </p>
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {[
              { label: 'Create new project', href: '/dashboard/projects', icon: '➕' },
              { label: 'Test your chatbot', href: '/dashboard/projects', icon: '💬' },
              { label: 'View analytics', href: '/dashboard/analytics', icon: '📊' },
              { label: 'Manage billing', href: '/dashboard/billing', icon: '💳' },
              { label: 'Try free tools', href: '/tools', icon: '🛠️' },
            ].map(action => (
      ...
      {org?.subscription_plan === 'free' && (
        <div className="mt-6 bg-gradient-to-r from-brand-600 to-purple-600 rounded-2xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-bold text-lg">Upgrade to Starter</p>
              <p className="text-brand-200 text-sm mt-1">
                Get 1,000 messages/month, 5 projects, and email support for just $29/mo.
              </p>
            </div>
            <Link
              href="/dashboard/billing"
              className="flex-shrink-0 bg-white text-brand-600 font-semibold px-5 py-2.5 rounded-xl hover:bg-brand-50 transition-colors text-sm"
            >
              Upgrade now →
            </Link>
          </div>
