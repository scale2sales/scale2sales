    ...
    <div className="p-8 max-w-5xl mx-auto">
        ...
        <div className="mb-6 bg-green-50 border border-green-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-green-800">
              Welcome to {successPlan ? PLAN_DETAILS[successPlan]?.name : 'your new plan'}!
            </p>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-10">
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              {org?.subscription_status === 'trialing' && (
                <span className="text-xs text-yellow-600 font-medium">Trial active</span>
              )}
          ...
          {currentPlan !== 'free' && (
            <button onClick={handlePortal} className="text-sm text-brand-600 hover:underline">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-4">This month usage</h2>
          <div className="space-y-3">
            <div>
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">Messages</span>
                <span className="font-medium text-gray-900">
                  {usage.messages.toLocaleString()} / {planLimit.toLocaleString()}
                </span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
                  className={`h-2 rounded-full ${usagePercent > 90 ? 'bg-red-500' : usagePercent > 70 ? 'bg-yellow-500' : 'bg-brand-500'}`}
                  style={{ width: `${usagePercent}%` }}
                />
              </div>
              {usagePercent > 80 && (
                <p className="text-xs text-yellow-600 mt-1">
                  Warning: {Math.round(usagePercent)}% used - upgrade to avoid interruption
                </p>
              )}
            </div>
      ...
      <div className="flex flex-col items-center mb-8">
        ...
        {billingInterval === 'year' && (
          <p className="text-sm text-green-600 font-medium mt-2">
            Save ${(29 - 19) * 12}/yr on Starter and ${(99 - 64) * 12}/yr on Pro
          </p>
        )}
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {Object.entries(PLAN_DETAILS).map(([key, plan]) => {
          ...
          return (
            <div
              key={key}
              className={`relative rounded-2xl border-2 p-6 flex flex-col ${plan.popular ? 'border-brand-500 shadow-lg' : 'border-gray-200'}`}
            >
              {plan.popular && (
              ...
              <div className="mb-4">
                ...
                {billingInterval === 'year' && plan.annualTotal && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    ...
                    <span className="text-green-600 font-medium">(save ${(plan.monthlyPrice - plan.annualPrice) * 12}/yr)</span>
                  </p>
                )}
                {billingInterval === 'month' && plan.monthlyPrice > 0 && (
                  <p className="text-xs text-gray-400 mt-0.5">
                    Or <span className="text-green-600 font-medium">${plan.annualPrice}/mo</span> billed annually
                  </p>
                )}
                {plan.monthlyPrice > 0 && (
                  <p className="text-xs text-green-600 font-medium mt-1">14-day free trial included</p>
                )}
              </div>
              ...
              {isCurrent ? (
                ...
                <button
                  ...
                  disabled={!!loading}
                  className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-colors disabled:opacity-50 ${plan.popular ? 'bg-brand-600 text-white hover:bg-brand-700' : 'bg-gray-900 text-white hover:bg-gray-800'}`}
                >
      ...
      <div className="mt-12">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: 'How does annual billing work?', a: 'You pay for the full year upfront and save 35%. Starter is $226/yr and Pro is $773/yr.' },
            ...
            { q: 'Can I switch from monthly to annual?', a: 'Yes! Click Manage billing to switch anytime. We will prorate the difference.' },
          ].map(item => (
