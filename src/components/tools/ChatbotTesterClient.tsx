...
function renderMarkdown(text) {
  ...
  return text
    ...
    .replace(/(<li[^>]*>[\s\S]+?<\/li>\s*)+/gm, (m) => `<ul style="margin:4px 0;padding-left:14px;list-style:disc">${m}</ul>`)
    .replace(/\n\n/g, '<br/>')
    .replace(/\n/g, ' ')
}
...
export function ChatbotTesterClient() {
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (data.type === 'complete') {
                ...
                setScanning(false)
                setMessages([{ id: crypto.randomUUID(), role: 'assistant', content: `Hi! I've just read **${data.pagesScanned} pages** from **${new URL(normalizedUrl).hostname}**. Ask me anything about this website!` }])
  ...
  async function sendMessage() {
    ...
    try {
      ...
      setMessages(prev => prev.map(m => m.id === assistantId ? { ...m, content: 'Sorry, something went wrong.', isStreaming: false } : m))
    }
    setChatLoading(false)
  }

  const suggestedQuestions = ['What services do you offer?', 'What are your prices?', 'How do I contact you?', 'What makes you different?']

  return (
    <div className="max-w-3xl mx-auto">
      <div className="bg-white rounded-2xl border border-gray-200 p-6 mb-6 shadow-sm">
        <label className="block text-sm font-semibold text-gray-700 mb-3">Enter any website URL to test</label>
        <div className="flex gap-3">
          <input type="text" value={url} onChange={e => setUrl(e.target.value)} onKeyDown={e => e.key === 'Enter' && !scanning && handleScan()}
          ...
          <button onClick={handleScan} disabled={scanning || !url.trim()}
            ...
            {scanning ? (<><svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Scanning…</>) : websiteReady ? '🔄 Re-scan' : '🚀 Scan & Chat'}
          </button>
        </div>
        {scanning && (
          <div className="mt-4 space-y-2">
            <div className="flex justify-between text-xs text-gray-500"><span>{scanMessage}</span><span>{scanProgress}%</span></div>
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div className="bg-brand-500 h-2 rounded-full transition-all duration-500" style={{ width: `${scanProgress}%` }}/>
            </div>
          </div>
        )}
        {scanError && <p className="mt-3 text-sm text-red-600 bg-red-50 px-3 py-2 rounded-lg">{scanError}</p>}
        {websiteReady && (
          <div className="mt-3 flex items-center gap-2 text-sm text-green-700 bg-green-50 px-3 py-2 rounded-lg">
            <span>✅</span><span>Scanned <strong>{pagesScanned} pages</strong> from <strong>{new URL(scannedUrl).hostname}</strong> — chatbot ready!</span>
          </div>
        )}
      </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-brand-100 flex items-center justify-center">
              <svg className="w-5 h-5 text-brand-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
            </div>
            <div>
            ...
            <div className="ml-auto flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse"/><span className="text-xs text-gray-500">Online</span>
            </div>
          </div>
          ...
          <div className="h-80 overflow-y-auto px-5 py-4 space-y-3 bg-gray-50">
            {messages.map(msg => (
              <div key={msg.id} className={`flex items-end gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${msg.role === 'user' ? 'bg-brand-600 text-white rounded-br-sm' : 'bg-white border border-gray-100 shadow-sm rounded-bl-sm text-gray-800'}`}>
                  {msg.role === 'user' ? <span style={{ lineHeight: '1.5' }}>{msg.content}</span>
                    : msg.content ? <div style={{ fontSize: '13px', lineHeight: '1.5' }} dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.content) }}/>
            ...
            <div className="px-5 py-3 border-t border-gray-100 flex flex-wrap gap-2">
              {suggestedQuestions.map(q => (
                <button key={q} onClick={() => { setInput(q); setTimeout(sendMessage, 50) }}
                  className="text-xs px-3 py-1.5 rounded-full border border-brand-200 text-brand-600 hover:bg-brand-50 transition-colors">{q}</button>
              ))}
            </div>
          )}
          ...
          <div className="px-5 py-4 border-t border-gray-100 flex gap-3">
            ...
            <button onClick={sendMessage} disabled={!input.trim() || chatLoading}
              className="px-4 py-2.5 rounded-xl bg-brand-600 text-white text-sm font-medium hover:bg-brand-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">Send</button>
          </div>
